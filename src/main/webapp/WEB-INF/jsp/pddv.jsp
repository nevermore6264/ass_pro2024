    ...
    <strong>${message }</strong>
            ...
            <img align="right" style="-webkit-filter: drop-shadow(1px 1px 2px #888888); filter: drop-shadow(1px 1px 2px #888888);margin-bottom: 10px;"
               ...
               <div style="float: left; width: 300px; height: 120px; margin: 10px;margin-right:15px;margin-left:20px ;display: inline-block;">
							<a class="hovera" href="dv?madatphong=${u.maDatPhong }&sophong=${u.phong.soPhong }" >
							<img style="-webkit-filter: drop-shadow(1px 1px 2px #888888); filter: drop-shadow(1px 1px 2px #888888);float: left;"
								src='<c:if test="${u.phong.trangThai==0 }">hinh/homegreen.png</c:if><c:if test="${u.phong.trangThai==1 }">hinh/homered.png</c:if>'
								width="120px" height="120px">
								<div style="margin-top: 10px;margin-left: 130px;">
								<div><b>Phòng: </b>${u.phong.soPhong }</div>
								<div><b>Loại: </b><c:if test="${u.phong.loaiPhong.tenLoaiPhong=='VIP'}"><img src="hinh/vip.png" width="55px" height="23px" style="margin-bottom: 5px;-webkit-filter: drop-shadow(1px 1px 1px #fff0b6); filter: drop-shadow(1px 1px 1px #fff0b6);"></img></c:if><c:if test="${u.phong.loaiPhong.tenLoaiPhong=='Thường'}">Thường</c:if></div>
								<div><b>Giảm giá: </b>${u.phong.khuyenMai } %</div>
								<div><b>Trạng thái: </b><c:if test="${u.phong.trangThai==0 }">Trống</c:if><c:if test="${u.phong.trangThai==1}">Đã thuê</c:if></div>
								</div>
							</a>
