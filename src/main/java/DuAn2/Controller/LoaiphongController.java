...
model.addAttribute("titlepage", "Danh sách loại phòng"); // tiêu đề cho trang
		...
		model.addAttribute("lLoaiphongs", phantranglp(vitrihientai,llp)); //hiện danh sách đã phân trang 10 item 1 trang
	...
	public String timdslp(ModelMap model,@ModelAttribute("loaiphong")LoaiPhong loaiphong, HttpServletRequest httpServletRequest) {
		activemenu(model);
	...
	public String actionaddlp(ModelMap model,@Validated @ModelAttribute("loaiphong")LoaiPhong loaiphong,BindingResult errors) {
		//cai dat. tat ca duoi deu show tren cai dat --------------------
				model.addAttribute("chamshowcd",".show");
					//show quan ly tai khoan
				model.addAttribute("chamshowqltk",null);
	...
	public String edit(ModelMap model,@Validated @ModelAttribute("loaiphong")LoaiPhong loaiphong,BindingResult errors) {
		activemenu(model);
		if(errors.hasErrors()) {
			model.addAttribute("titlepage", "Thêm mới loại phòng");
			model.addAttribute("errors", errors.getAllErrors());
	...
	public String delete(ModelMap model,@ModelAttribute("loaiphong")LoaiPhong loaiphong,@RequestParam("maLoaiPhong")Integer maLoaiPhong) {
		activemenu(model);
...
List<LoaiPhong> l = (List<LoaiPhong>) iLoaiphongServices.findAll();
		...
		if(phantranglp(vitrihientai,l).isEmpty()) {
	...
	//-------------
		...
		public String lppage(ModelMap model,@RequestParam("page")int page,@ModelAttribute("loaiphong")LoaiPhong loaiphong) {
			List<LoaiPhong> l = (List<LoaiPhong>) iLoaiphongServices.findAll();
			activemenu(model);
			vitrihientai = page;
		...
		//-------------
			...
			public String lppagetim(ModelMap model,@RequestParam("page")int page,@ModelAttribute("loaiphong")LoaiPhong loaiphong) {
				activemenu(model);
				model.addAttribute("titlepage", "Danh sách loại phòng");
				vitrihientai = page;
